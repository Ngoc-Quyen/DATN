...
const BOOKING_IMAGE_URL =
    'http://ipright.vn/wp-content/uploads/2014/03/36322201-procedure-word-write-on-paper-Stock-Photo-1200x545_c.jpg';
...
const DEFAULT_IMAGE_URL =
    'https://www.freseniusmedicalcare.com.vn/fileadmin/_processed_/5/4/csm_SPE001_service-support-employee_7614d83ad5.jpg';
    ...
    switch (name) {
        case 'intent':
            ...
            if (entity.value === 'tiêu hóa') {
                await callSendAPI(
                    sender_psid,
                    'Bạn đang gặp vấn đề về bệnh đường tiêu hóa, xem thêm danh sách bác sĩ chuyên khoa TIÊU HÓA.'
                );
            ...
            if (entity.value === 'cơ-xương-khớp') {
                await callSendAPI(
                    sender_psid,
                    'Bạn đang gặp vấn đề về cơ-xương-khớp, xem thêm danh sách bác sĩ chuyên khoa CƠ XƯƠNG KHỚP.'
                );
                let title = 'Chuyên khoa khám bệnh';
        ...
        case 'booking':
            await callSendAPI(
                sender_psid,
                'Bạn đang cần đặt lịch khám bệnh, xem thêm hướng dẫn đặt lịch chi tiết ở link bên dưới nhé.'
            );
            await callSendAPIv2(
                sender_psid,
                'Đặt lịch khám bệnh',
                'Hướng dẫn đặt lịch khám bệnh tại Doctors Care',
                BOOKING_IMAGE_URL,
                BOOKING_URL
            );
            await sendMessageDefault(sender_psid, 'Xem thêm thông tin:');
        ...
        case 'info':
            ...
            await callSendAPIv2(
                sender_psid,
                'Thông tin website',
                'Thông tin website Doctors care',
                INFOWEBSITE_IMAGE_URL,
                INFOWEBSITE_URL
            );
            await sendMessageDefault(sender_psid, 'Xem thêm thông tin:');
            break;
        default:
            await callSendAPI(
                sender_psid,
                'Rất tiếc bot chưa được hướng dẫn để trả lời câu hỏi của bạn. Để được hỗ trợ, vui lòng truy câp:'
            );
            await callSendAPIv2(
                sender_psid,
                'Hỗ trợ khách hàng',
                'Thông tin hỗ trợ khách hàng Doctors care',
                DEFAULT_IMAGE_URL,
                DEFAULT_URL
            );
            await sendMessageDefault(sender_psid, 'Xem thêm thông tin:');
...
let sendMessageMedicalExamination = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                text:
                    'Quy trình khám bệnh với Doctor Care gồm những bước sau:' +
            ...
            let response2 = {
                attachment: {
                    ...
                    payload: {
                        ...
                        text:
                            'Để được hỗ trợ thông tin cũng như đặt lịch khám bệnh online,' +
...
let sendMessageReplyDoctors = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                text:
                    'Doctors Care tự hào mang đến cho bạn đội ngũ bác sĩ hàng đầu, chất lượng và uy tín.' +
                    ...
                    payload: {
                        ...
                        elements: [
...

                            {
                                ...
                                image_url:
                                    'https://doctorcare-v1.herokuapp.com/images/users/doctor-hoang-dinh-tung.jpg',
...
let sendMessageReplySpecialization = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                text:
                    'Doctors Care có đầy đủ các chuyên khoa khám chữa bệnh cho mọi lứa tuổi.' +
                        ...
                        elements: [
                            {
                                title: 'TAI MŨI HỌNG',
                                image_url:
                                    'https://doctorcare-v1.herokuapp.com/images/specializations/tai-mui-hong.jpg',
                                default_action: {
                                    type: 'web_url',
...
let sendMessageAllDoctors = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                text:
                    'Danh sách tất cả bác sĩ bạn xem thêm ở link bên dưới nhé:' +
...
let sendMessageAllSpecializations = (sender_psid) => {
    return new Promise(async (resolve, reject) => {
        try {
            let response1 = {
                text:
                    'Danh sách tất cả các chuyên khoa bạn xem thêm ở link bên dưới nhé:' +
